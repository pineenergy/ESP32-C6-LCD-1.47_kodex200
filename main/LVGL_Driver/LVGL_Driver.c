...
void example_lvgl_flush_cb(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *color_map)
{
    ...
    esp_lcd_panel_draw_bitmap(panel_handle,
                              offsetx1 + panel_offset_x,
                              offsety1 + panel_offset_y,
                              offsetx2 + panel_offset_x + 1,
                              offsety2 + panel_offset_y + 1,
                              color_map);
}
...
void LVGL_Init(void)
{
    ...
    disp_drv.ver_res = EXAMPLE_LCD_H_RES; // Horizontal pixel count
    ...
    // Hardware-only landscape orientation: keeps LVGL layout in normal top-to-bottom flow.
    esp_lcd_panel_swap_xy(panel_handle, true);
    esp_lcd_panel_mirror(panel_handle, false, false);
